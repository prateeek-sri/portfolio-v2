...
export function DraggableCardDemo() {
  const items = [
    ...
    },
    {
      ...
      className: "absolute top-20 left-[40%] rotate-[-6deg]",
    },
    {
      title: "Kakashi Hatake",
      image:
        "https://i.pinimg.com/736x/6b/4b/cc/6b4bcc4bc8b66fc466310c29c2c99eeb.jpg",
      className: "absolute top-6 left-[25%] rotate-[3deg]",
    },
  ];

  return (
    <DraggableCardContainer className="relative flex min-h-[80vh] w-full items-center justify-center overflow-hidden">
      <p className="absolute top-1/2 mx-auto max-w-sm -translate-y-3/4 text-center text-xl font-bold text-neutral-400 md:text-3xl dark:text-neutral-800">
      ...
      {items.map((item) => (
        <DraggableCardBody key={item.title} className={item.className}>
          <img
            src={item.image}
            alt={item.title}
            className="pointer-events-none relative z-10 h-56 w-56 sm:h-64 sm:w-64 md:h-72 md:w-72 lg:h-80 lg:w-80 object-cover rounded-xl"
          />
